...
def get_db():
    if 'db' not in g: g.db = sqlite3.connect(current_app.config['DATABASE'], detect_types=sqlite3.PARSE_DECLTYPES); g.db.row_factory = sqlite3.Row
    return g.db
...
def add_cost(name, amount, description, cost_type, date_str):
    db = get_db()
    db.execute('INSERT INTO costs (name, amount, description, cost_type, created_at) VALUES (?, ?, ?, ?, ?)', (name, amount, description, cost_type, date_str)); db.commit()
...
def batch_add_costs(costs):
    db = get_db()
    db.executemany('INSERT INTO costs (name, amount, description, cost_type, created_at) VALUES (:name, :amount, :description, :type, :date)', costs); db.commit()

def delete_cost_by_id(cost_id): db = get_db(); db.execute('DELETE FROM costs WHERE id = ?', (cost_id,)); db.commit()
def delete_costs_by_type(cost_type, year, month): db = get_db(); db.execute("DELETE FROM costs WHERE cost_type = ? AND strftime('%Y', created_at) = ? AND strftime('%m', created_at) = ?", (cost_type, year, month)); db.commit()
def update_cost(cost_id, name, amount, description): db = get_db(); db.execute('UPDATE costs SET name = ?, amount = ?, description = ? WHERE id = ?', (name, amount, description, cost_id)); db.commit()
def update_cost_type(cost_id, new_type): db = get_db(); db.execute('UPDATE costs SET cost_type = ? WHERE id = ?', (new_type, cost_id)); db.commit()
def update_checked_status(cost_id, is_checked): db = get_db(); db.execute('UPDATE costs SET is_checked = ? WHERE id = ?', (is_checked, cost_id)); db.commit()

def get_budget(year, month):
    budget = get_db().execute('SELECT salary, fixed_percent, variable_percent FROM budgets WHERE year = ? AND month = ?', (year, month)).fetchone()
