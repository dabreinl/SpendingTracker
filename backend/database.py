...
def delete_cost_by_id(cost_id):
    db = get_db()
    db.execute('DELETE FROM costs WHERE id = ?', (cost_id,)); db.commit()

def delete_costs_by_type(cost_type, year, month):
    db = get_db()
    db.execute("DELETE FROM costs WHERE cost_type = ? AND strftime('%Y', created_at) = ? AND strftime('%m', created_at) = ?", (cost_type, year, month)); db.commit()
...
def update_cost_type(cost_id, new_type):
    db = get_db()
    db.execute('UPDATE costs SET cost_type = ? WHERE id = ?', (new_type, cost_id)); db.commit()