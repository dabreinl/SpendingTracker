...
def create_app():
    ...
    def get_budget(year, month):
        ...
        if budget: return jsonify(budget)
    ...
    def save_budget(year, month):
        ...
        if not data or not all(k in data for k in required): return jsonify({'error': 'Missing data'}), 400
        database.save_budget(year, month, data['salary'], data['fixed_percent'], data['variable_percent'])
        return jsonify({'success': True}), 200

    # --- Other routes remain unchanged ---
    @app.route('/api/costs/summary')
    def get_costs_summary(): return jsonify(database.get_summary_of_months())
    @app.route('/api/costs/all_previous')
    def get_all_previous():
        year, month = request.args.get('year'), request.args.get('month')
        if not all([year, month]): return jsonify({'error': 'Missing year/month parameters'}), 400
    ...
    def batch_add():
        ...
        if not isinstance(costs, list): return jsonify({'error': 'Request body must be a list'}), 400
        for cost in costs: cost['description'] = cost.get('description', None)
    ...
    def update_cost(cost_id):
        data = request.get_json(); required = ['name', 'amount']
    ...
    def clear_costs(cost_type):
        year, month = request.args.get('year'), request.args.get('month')
        if not all([year, month]): return jsonify({'error': 'Missing year/month parameters'}), 400
        if cost_type not in ['fixed', 'variable']: return jsonify({'error': 'Invalid cost type'}), 400
    ...
    def update_checked(cost_id):
        data = request.get_json()
        if 'is_checked' not in data or not isinstance(data['is_checked'], bool): return jsonify({'error': 'Invalid or missing is_checked field'}), 400
